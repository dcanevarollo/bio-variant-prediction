...
Sequences containing the character "X" (unknown or non-standard amino acid) were not filtered at this stage; this filtering was applied downstream, immediately before protein language model inference.
After all quality-control steps described above, the resulting dataset comprised 76,990 variants across 8,297 unique genes.
...
Table~\ref{tab:comparative} summarizes the median ROC-AUC and PR-AUC, evaluated per gene over the evaluable gene set, for the four representation--model combinations considered in this study.

\begin{table}[ht]
\centering
\caption{Comparative zero-shot performance across the four representation--model combinations, ordered by descending median ROC-AUC.}
\label{tab:comparative}
\begin{tabular}{lccc}
\hline
\textbf{Representation} & \textbf{Median ROC-AUC} & \textbf{Median PR-AUC} & \textbf{Genes} \\
\hline
ESM-2 log-ratio     & \textbf{0.951} & \textbf{0.983} & 2,252 \\
Prot-T5 delta-norm  & 0.635          & 0.692          & 2,252 \\
Prot-T5 log-ratio   & 0.484          & 0.565          & 2,252 \\
ESM-2 delta-norm    & 0.450          & 0.591          & 2,252 \\
\hline
\end{tabular}
\end{table}

(Of the 2,253 genes meeting the evaluability criterion defined in Section \ref{sec:dataset}, scores were available for all four representations for 2,252; the remaining gene, missing a score for at least one representation, was excluded from this comparison.)

The ESM-2 log-ratio representation clearly outperforms all other combinations, and no delta-embedding score, for either model, approaches it. Interestingly, the relative ranking of log-ratio versus delta-embedding is not consistent across models: for ESM-2, log-ratio greatly outperforms delta-norm, whereas for Prot-T5, delta-norm outperforms log-ratio. Both delta-norm comparisons (Prot-T5 delta-norm vs.\ Prot-T5 log-ratio, and ESM-2 delta-norm vs.\ Prot-T5 delta-norm) are statistically significant under a paired Wilcoxon signed-rank test ($p = 8.23\times10^{-55}$ and $p = 6.80\times10^{-104}$, respectively). A direct paired test between ESM-2 log-ratio and ESM-2 delta-norm --- the pair with the largest performance gap in this study --- was not formally computed (see Section \ref{sec:limitations}); given the magnitude of the median difference between the two (0.951 versus 0.450), however, a formal test would be unlikely to alter the qualitative conclusion for this pair. Taken together, this suggests that the relative merit of probability-derived versus embedding-shift-derived scores depends on the specific model architecture, rather than being an intrinsic property of either representation family on its own.

\subsection{The ESM-2 Delta-Embedding Result: A Below-Chance Median}

The most unexpected result in this study is that ESM-2 delta-norm achieves a median ROC-AUC of 0.450 --- below the level expected by chance. Figure~\ref{fig:esm2delta-dist} shows why this median is misleading if read as a single number: rather than a uniformly weak signal, the per-gene ROC-AUC distribution is concentrated at three distinct points --- near 0.0, near 0.5, and near 1.0 --- indicating that the representation is strongly correct for some genes, strongly anti-correlated for others, and uninformative for the rest.

A natural concern is whether this dispersed, multi-peaked shape simply reflects the coarse set of ROC-AUC values achievable with small per-gene sample sizes --- the median number of variants per gene in the evaluable set is 3, only slightly above the evaluability floor of 5 variants spanning both classes (Section \ref{sec:dataset}) --- rather than genuine heterogeneity in representation quality across genes. If this discreteness were the dominant explanation, a similarly dispersed, multi-peaked distribution would be expected for any representation evaluated over the same genes and the same per-gene sample sizes, regardless of its discriminative quality. Figure \ref{fig:esm2-logratio-dist} shows the per-gene ROC-AUC and PR-AUC distributions for the ESM-2 log-ratio representation, evaluated over the same evaluable gene set. In contrast to ESM-2 delta-norm, the log-ratio distribution is strongly unimodal and concentrated near 1.0, with a comparatively short left tail and no corresponding accumulation of genes near 0.0 or 0.5. Since both representations are scored under the same per-gene sample-size constraints, this contrast indicates that the dispersed, multi-peaked shape observed for ESM-2 delta-norm is not primarily a sampling artifact, and instead reflects genuinely heterogeneous behavior of this representation across genes.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig_esm2_delta_auc_distributions.pdf}
\caption{Per-gene distribution of ROC-AUC (left) and PR-AUC (right) for the ESM-2 delta-norm representation.}
\label{fig:esm2delta-dist}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{fig_esm2_auc_distributions.pdf}
\caption{Per-gene distribution of ROC-AUC (left) and PR-AUC (right) for the ESM-2 log-ratio representation, shown for contrast with Figure \ref{fig:esm2delta-dist}.}
\label{fig:esm2-logratio-dist}
\end{figure}

Before treating this as a genuine finding rather than an indexing bug, the embedding-extraction alignment was manually checked on a small sample of variants, confirming that the residue retrieved at the computed extraction index matched the annotated wild-type amino acid in all cases tested. A sign-inversion test ($1 - \text{ROC-AUC}$ per gene) yielded a median of 0.5502, indicating that benign variants tend to produce a \emph{larger} embedding shift than pathogenic ones at the final layer of ESM-2 --- the opposite of the design hypothesis that motivated the delta-embedding representation.

A plausible, though untested, explanation is that the final layer of ESM-2 is specialized toward its masked-token-prediction pretraining objective and may capture local evolutionary plausibility of a residue rather than the structural severity of a substitution; an intermediate layer might preserve the pathogenic/benign distinction better, but this was not evaluated here and is left for future work. This result is reported as an informative contrast between architectures --- the same scoring strategy works very differently for ESM-2 and Prot-T5 --- rather than as a failure of the delta-embedding approach.

\subsection{Limitations}
\label{sec:limitations}

The analysis presented here is restricted to gnomAD chromosome 1 exome data, which limits the size and representativeness of the evaluable gene set. ClinVar is also known to be subject to ascertainment bias with respect to allele rarity \cite{mikhail}, which may affect the pathogenicity label distribution against which all representations were scored. Finally, a direct paired significance test between ESM-2 log-ratio and ESM-2 delta-norm --- the two representations with the largest performance gap in this study --- was not performed; additional analyses, such as performance stratified by the number of variants per gene and correlations with gene-level covariates, were also left for future work.

\section{Conclusions}

This paper compared two zero-shot mutation representation strategies --- log-ratio scores derived from model output probabilities, and delta-embedding norms derived from hidden-state shifts --- across two pretrained protein language models, ESM-2 and Prot-T5, for predicting the pathogenicity-linked functional impact of human missense variants in a per-gene low-data regime. Using ClinVar as the source of pathogenicity labels and gnomAD chromosome 1 to define the dataset's scope, all four representation--model combinations were evaluated under an explicit per-gene evaluability criterion, without any supervised fine-tuning or downstream classifier. The ESM-2 log-ratio representation clearly outperformed every other combination (median ROC-AUC of 0.951), while no delta-embedding score, for either model, approached this level of performance. Notably, the relative ranking between the two representation families was not consistent across models --- log-ratio dominated delta-norm for ESM-2, while the opposite held for Prot-T5 --- indicating that the effectiveness of a given representation strategy depends on the specific architecture it is extracted from, rather than being a fixed property of the representation itself.

The most unexpected result was that the ESM-2 delta-embedding score achieved a below-chance median ROC-AUC, with benign variants tending to produce a larger embedding shift than pathogenic ones at the model's final layer. After ruling out an indexing artifact as the cause, this result was interpreted as a genuine architectural contrast rather than a failure of the delta-embedding approach: the same general strategy behaves very differently depending on the underlying model, which is itself an informative finding for future work on embedding-based variant scoring.

A caveat applies to all comparisons involving log-ratio scores across the two models: because ESM-2's log-ratio score is computed with the wild-type residue visible in context, while Prot-T5's is computed under masking, the two scores are not extracted under equivalent conditions and were therefore never compared in absolute magnitude in this study, only through ranking-based metrics evaluated independently for each model. This asymmetry should be kept in mind when interpreting any claim of one model being intrinsically ``better'' than the other for this task, since part of the observed gap may reflect the scoring protocol rather than the underlying representation quality alone.

Overall, these results suggest that the choice of mutation representation and the choice of protein language model interact substantially, and that neither factor alone is sufficient to predict zero-shot performance in low-data, per-gene pathogenicity prediction. The limitations and directions for future work discussed in the previous sections --- in particular, extending beyond chromosome 1, incorporating allele frequency as a model feature, and evaluating intermediate model layers --- remain the most direct paths toward a more complete understanding of this comparison.

\section{Future Work}

Several directions remain open beyond the scope of this work. First, and most directly tied to the scope limitation already noted in the Abstract and in Results and Discussion, extending the analysis beyond gnomAD chromosome 1 would allow the evaluable gene set to grow and would remove the chromosome-specific constraints on allele-frequency-derived statistics. Second, incorporating population allele frequency as an explicit model feature --- rather than as the single-feature baseline considered here --- in hybrid representations that combine pLM-derived scores with $\log\_AF$ is a natural extension, particularly given that allele frequency and pLM-based scores capture different and potentially complementary sources of evidence.

Third, the below-chance result obtained for ESM-2 delta-norm motivates evaluating intermediate layers of the model, rather than only the final layer used here, since the final layer's proximity to the masked-token-prediction objective may explain why it does not behave as a severity proxy. Fourth, a direct paired significance test between ESM-2 log-ratio and ESM-2 delta-norm --- the comparison with the largest performance gap in this study, but the one for which no formal test was computed --- would close the most relevant gap in the statistical analysis presented here. The additional stratified and correlational analyses mentioned as limitations above also remain natural candidates for a more complete follow-up.

Finally, this work intentionally restricts its scope to zero-shot scoring, without any supervised component. Evaluating these same representations as input features to lightweight supervised classifiers in a few-shot, per-gene regime --- an approach considered during the design of this study but excluded to keep the comparison strictly zero-shot --- could clarify whether the gap between log-ratio and delta-embedding representations persists, narrows, or reverses once even a small amount of gene-specific supervision is introduced.
